        ...
        authenticate("azureAuth") {
            post{
                ...
                try{
                    ...
                    secureLogger.info { "Antall vurderinger found in db for ${request.fnr} is : ${vurderinger.size}"
                        kv("fnr",request.fnr)
                        kv("callId", callId)
                    }
                    ...
                    if (vurdering.isEmpty){
                        secureLogger.info { "cache miss for  ${request.fnr}"
                            kv("fnr",request.fnr)
                            kv("callId", callId)
                            kv("periode", periode)
                        }
                        ...
                        secureLogger.info { "cache hit for  ${request.fnr}"
                            kv("fnr",request.fnr)
                            kv("callId", callId)
                            kv("periode", periode)
